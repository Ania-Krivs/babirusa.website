...
### [Демонстрация работы проекта](https://babirusa.website)
...
- ssl-сертификат на домен (например, через certbot), настройка nginx на сервере для https и mitmproxy (пример конфига для домена babirusa.website лежит в корне проекта)
...
### /frontend/.env
```
VITE_BACKEND_URL="https://api.babirusa.website"
VITE_ADMIN_PANEL_PASSWORD="change_me"
```
где https://api.babirusa.website - адрес бэкенда, может быть установлен просто как 127.0.0.1.
### /backend/.env
```
